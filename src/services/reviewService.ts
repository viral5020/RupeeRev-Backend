  ...
  try {
    ...
    if (!isEligible) {
      return {
        eligible: false,
        reason:
          "Only premium users or users who have purchased tokens can submit reviews",
      };
    }
...
export const createReview = async (
  ...
  try {
    ...
    const existingReview = await Review.findOne({ userId }).lean();
    if (existingReview) {
      throw new Error(
        "You have already submitted a review. You can edit your existing review."
      );
    }
